...
class TmuxOrchestrator:
    ...
    def get_tmux_sessions(self) -> List[TmuxSession]:
        ...
        If tmux is not installed the method returns an empty list and
        prints a warning.  Any other errors during invocation of tmux
        commands are also reported and result in an empty list.
        """
        try:
            # Ask tmux for the list of sessions.  The format string returns
            # the session name and whether it is attached.  Using
            # ``capture_output=True`` avoids printing to stdout/stderr.
            sessions_cmd = [
                "tmux",
                "list-sessions",
                "-F",
            ...
            for line in sessions_result.stdout.strip().split('\n'):
                ...
                # For each session query its windows.  The format string
                # returns the index, name and active status for each window.
                windows_cmd = [
                    "tmux",
                    "list-windows",
                    "-t",
                ...
                for window_line in windows_result.stdout.strip().split('\n'):
                    if not window_line:
                        continue
                    # Split only on the first two colons; window names can
                    # themselves contain colons.
                    parts = window_line.split(":", 2)
                    if len(parts) != 3:
                        continue
                    window_index_str, window_name, window_active = parts
                    windows.append(
                        TmuxWindow(
            ...
            # ``tmux`` could not be found on the system.  Inform the caller
            # and return no sessions so that callers can degrade gracefully.
            print(
                "Warning: 'tmux' executable not found. Install tmux to use the "
                "orchestrator features."
            )
            return []
        except subprocess.CalledProcessError as e:
            # ``tmux`` returned a non‑zero exit code.  Print the error and
            # return no sessions.  This could happen if no tmux server is
            # running.
